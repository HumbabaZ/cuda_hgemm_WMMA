...
__global__ void wmmaAsyncKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C, size_t M,
                                size_t N, size_t K) {
...
#pragma unroll
        for (size_t i = 0; i < A_smem_iters; ++i) {
            //Async Copy
            //directly transfer data from global memory to shared memory
            //doesn't block the calculation threads
            //__cvta_generic_to_shared:shared memory adress
            uint32_t A_smem_lane_addr =
                __cvta_generic_to_shared(&smem[A_smem_idx][0]) + (lane_id % CHUNK_COPY_LINE_LANES) * THREAD_COPY_BYTES;

            //decrease dependency on __syncthreads()
            //calculation doesn't need to wait for data transfering
...
size_t initWmmaAsync() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void wmmaAsync(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    ...
    wmmaAsyncKernel<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
