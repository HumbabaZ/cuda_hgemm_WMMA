...
//
// Description: wmma padding hgemm
...
__global__ void wmmaPaddingKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C,
...
size_t initWmmaPadding() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaPaddingKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void wmmaPadding(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t smem_max_size = initWmmaPadding();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    wmmaPaddingKernel<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
